...
export class Vehicle {
  ...
  removeBookingByUUID(bookingUuid) {
    if (bookingUuid) {
      ...
      const bookingToBeDeleted = bookingsCopy.find(
        (booking) => booking.uuid === bookingUuid
      );

      if (bookingToBeDeleted) {
        this.bookings = bookingsCopy.filter(
          (booking) => booking.uuid !== bookingToBeDeleted.uuid
        );
        this.updateVehicleOdometer(null, false);
      }
    }
  ...
  addJourney(newJourney) {
    this.bookings
      .find((b) => b.uuid === newJourney.bookingUuid)
      .addJourney(newJourney);
  ...
  removeServiceByUUID(serviceUuid) {
    if (serviceUuid) {
      ...
      const serviceToBeDeleted = servicesCopy.find(
        (service) => service.uuid === serviceUuid
      );

      if (serviceToBeDeleted) {
        this.services = servicesCopy.filter(
          (service) => service.uuid !== serviceToBeDeleted.uuid
        );
      }
    }
  }
  ...
  addFuelPurchase(newFuelPurchase) {
    this.bookings
      .find((b) => b.uuid === newFuelPurchase.bookingUuid)
      .addFuelPurchase(newFuelPurchase);
  ...
  printDetails() {
    return {
      ...
      "Revenue recorded": `$ ${Number.parseFloat(
        this.calculateRevenueRecorded()
      ).toFixed(2)}`,
      "Kilometers since the last service": Number.parseFloat(
        Service.getLastServiceOdometerReading(this.services)
      )
        ? `${
            this.odometer - Service.getLastServiceOdometerReading(this.services)
          } km`
        : Service.getLastServiceOdometerReading(this.services),
  ...
  calculateFuelEconomy() {
    const fuelPurchases = this.bookings.reduce((fuelPurchases, booking) => {
      fuelPurchases.push(...booking.fuelPurchases);
      return fuelPurchases;
    }, []);

    const totalFuelPurchaseCost = fuelPurchases.reduce(
      (totalCost, fuelPurchase) => {
    ...
    if (
      Number.isNaN(totalFuelPurchaseCost) ||
      Number.isNaN(totalDistanceTravelled) ||
      !Number.isFinite(totalFuelPurchaseCost / totalDistanceTravelled)
    ) {
  ...
  calculateTotalDistanceTravelled() {
    return this.bookings.reduce((totalDistance, booking) => {
      ...
      if (this.bookings.length) {
        const todaysJourneys = this.bookings.reduce((j, b) => {
          if (b.journeys.length) {
            j.push(
              ...b.journeys.filter((j) =>
                moment(j.endedAt).isSame(moment(), "days")
              )
            );
          }
          return j;
        ...
        const greatestEndOdometer = todaysJourneys.reduce(
          (greatestEndOdometer, j) => {
        ...
        this.odometer = greatestEndOdometer
          ? greatestEndOdometer
          : this.odometer;
      }
    }
